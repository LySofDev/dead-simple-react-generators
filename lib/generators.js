...
function generateServiceFile(workDir, identifier) {
  const fileName = voca.kebabCase(identifier);
  const componentName = voca.capitalize(voca.camelCase(identifier));
  const templateFilePath = `${templatesDirectory}/template.service.ts`;
  const targetFilePath = `${workDir}/${fileName}.service.ts`;
  if (fs.existsSync(targetFilePath)) {
    console.log(`Skipped creating service: ${targetFilePath}`);
    return targetFilePath;
  }
  let renderedTemplate = fs.readFileSync(templateFilePath, 'utf-8');
  renderedTemplate = renderedTemplate.split('Template').join(componentName);
...
function generateComponent(workDir, identifier, flat) {
  ...
  if (flat) {
    componentDirectory = workDir;
  } else {
    componentDirectory = `${workDir}/${voca.kebabCase(identifier)}`;
